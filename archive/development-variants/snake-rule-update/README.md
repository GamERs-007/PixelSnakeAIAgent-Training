...
## Dense-board sweep rule

Select `ultimate/2300_fill50_model.pt` with **Trap avoidance assistance (all AI)**
enabled. This variant retains the weights and training state from
`strategy_v1/2300_model.pt` and adds the checkpoint metadata flag
`inference_rules.dense_board_sweep_above_half`. The legacy filename and flag are
retained for compatibility; the browser's current threshold is **60%**.
The browser reads that flag;
other models keep their existing behavior. A snake occupying strictly
more than 60% of the board switches to a row-by-row sweep (241 cells on the 20x20
board). It crosses the top row to the right, alternates left/right on successive
rows, and reserves the left column for returning to the top. Eating keeps this
mode active; restarting a round resets it. The trigger is body occupancy, not
time without scoring. The AI status shows whether it is sweeping or joining.

At or below 60%, a shortest-path search prioritizes reaching food with fewer
steps and verifies that the body can escape after eating. It does not require
early moves to preserve the future sweep's order. If that path is unsafe or
unavailable, the existing space and loop assistance selects a fallback.
Above 60%, the guard checks the entire body's cyclic order before following
the sweep. Disordered bodies use safe food paths and reduced winding to join;
this is best effort and does not guarantee a full-board win. The earlier
eight-seed win result applied to the previous order-preserving policy, not
this shortest-path policy. Restart the round after updating. No retraining
or checkpoint replacement is required.
