...
(function(root) {
  ...
  function orderedCycleAction(state, preferred, filling) {
    ...
    // Below 50%, retain DQN choices when they preserve the body's cyclic order.
    // Never jump past the fruit or overtake the tail. Above 50%, no shortcuts.
  ...
  class SafetyGuard {
    ...
    choose(state, preferred) {
      ...
      const ordered=this.denseBoardSweep && fillAction(state)!==null && cycleSpan(state)<state.size*state.size;
      if (ordered) {
