  ...
  function updateStats(player) {
    ...
    if (safetyEnabled && player.agent.guard?.filling) {
      ...
      $('ai-status-' + player.id).textContent = language==='zh-CN'
        ? (joining ? '蛇身超过 60% · 正在安全接入铺满路线' : '蛇身超过 60% · 逐行铺满模式')
        : language==='fr' ? (joining ? 'Occupation > 60 % · Rejoint le parcours' : 'Occupation > 60 % · Parcours ligne par ligne')
        : (joining ? 'Board > 60% · Joining sweep safely' : 'Board > 60% · Row-by-row sweep');
    }
