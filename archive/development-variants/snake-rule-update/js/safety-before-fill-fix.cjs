...
(function(root) {
  ...
  class SafetyGuard {
    ...
    choose(state, preferred) {
      ...
      this.filling=this.denseBoardSweep && state.snake.length>state.size*state.size/2 && fillAction(state)!==null;
      if (this.filling) this.plan=[];
      ...
      if(!this.filling && !this.plan.length){this.plan=safeFoodPath(state)||[];this.planFood=foodKey;}
      if(this.plan.length){
      ...
      if (this.filling) {
        // An arbitrary existing body may cross the cycle. Join it only through
        // legal moves with the best available tail/space safety rating.
        const cycle=eligible.find(o=>o.action===fillAction(state));
        const chosen=cycle || eligible.sort((a,b)=>b.area-a.area || visits(a)-visits(b) ||
          Number(b.action===preferred)-Number(a.action===preferred))[0];
        this.lastReason=cycle ? 'fill' : 'fill_join';
  ...
  const api={SafetyGuard,AssistedAgent,assess,safeFoodPath,fillAction};
