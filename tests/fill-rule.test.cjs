...
test('below 50%, nearby food uses the shortest safe path instead of a cycle detour',()=>{
...
test('exactly 50% still follows a safe food plan without enforcing cyclic order',()=>{
  const state=snapshot(200,250,true),guard=new SafetyGuard({denseBoardSweep:true});
...
test('strictly more than 50% triggers the sweep, regardless of score stagnation',()=>{
  const guard=new SafetyGuard({denseBoardSweep:true});
  for(const length of [199,200,201]) {
    const state=snapshot(length);
    guard.choose(state,'UP');
    assert.equal(guard.filling,length>200);
    if(length>200) assert.equal(guard.lastReason,'fill');
  }
  guard.reset();
  assert.equal(guard.filling,false);
...
test('an aligned dense snake follows the cycle through growth to a full board',()=>{
  const guard=new SafetyGuard({denseBoardSweep:true}),state=snapshot(201,397),cells=route(20);
  let head=397;
...
test('joining never forces reversal or an immediate collision into the existing body',()=>{
  const guard=new SafetyGuard({denseBoardSweep:true}),state=snapshot(201,250,true);
...
test('disabled assistance and pending inference keep their existing contracts',()=>{
  const state=snapshot(201),agent=new AssistedAgent({chooseAction:()=> 'UP'},()=>false);
...
test('recorded dense-board stalls unwind and reach their previously inaccessible food',()=>{
  for(const seed of [7,19]) {
    ...
    let ate=false;
    // Recovery may spend one lap joining, then one lap reaching food. The
    // strict 400-move food bound applies only AFTER genuine ordered lock.
    for(let i=0;i<800;i++) {
